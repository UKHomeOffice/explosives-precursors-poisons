...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const fileUploadStatusHandler = status => {
    const fileUploadComponent = document.getElementById('file-upload-group');
    ...
    switch (status) {
      ...
      case 'error':
        if (fileUploadComponent) {
          ...
          document.getElementById('file-upload-group').classList.remove('govuk-!-display-none');
        }
        break;
      case 'uploading':
